# Documentation map

## Start here

- [Project overview](../README.md)
- [Technical report](../report/README.md)
- [Research roadmap](../PLAN.md)
- [Current status and open queues](../PROGRESS.md)
- [Contributor guide](../CLAUDE.md)

## Data and methods

- [Dataset card](../data/dataset_card.md)
- [Session format](../specs/session_format.md)
- [Field semantics](../specs/field_semantics.md)
- [Frame-index encoding](../specs/frameindex_encoding.md)
- [Input-overlay contract](../specs/overlay_spec.md)
- [Wild20 production path](../harvest/WILD20.md)

## Results and evidence

Current results first:

- [Resolved-v3 production run](../results/idm/vpt_small_nitrogen210_resolved_v3_v1/RESULTS.md)
- [Wild7 deployment gate](../results/idm/VPT_SMALL_WILD_ADMITTED7_PRIMARY_GATE.md)
- [NitroGen label-incident record](../results/idm/NITROGEN_LABEL_INCIDENTS.md)
- [Wild7 checkpoint parity scorecard](../results/idm/vpt_wild7_checkpoint_parity_v1/README.md)

Historical and supporting:

- [IDM results summary](../results/idm/SUMMARY.md)
- [Key-state accuracy and baselines](../results/idm/KEYPRESS_ACCURACY.md)
- [NitroGen-only holdout](../results/idm/NITROGEN_HOLDOUT.md)
- [Full-corpus audit](../results/idm/CORPUS_AUDIT.md)
- [Training-data policy](../results/idm/TRAINING_DATA_POLICY.md)

## Publication

- [Third-party notices](../THIRD_PARTY_NOTICES.md)

## Durable engineering references

- [Curated engineering lessons](engineering-lessons.md)

## Historical records

Internal chronology preserved for the record, not current guidance:

- [How this was built](history/how-this-was-built.md)
- [2026-07-25 project review](history/2026-07-25-project-review.md)

Chronological logs retain the terminology used at the time. Current public
claims and naming are defined by the technical report and the results summary.
Volatile run and queue status lives only in [PROGRESS.md](../PROGRESS.md).
